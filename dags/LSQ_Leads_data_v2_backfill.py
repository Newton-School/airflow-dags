...
create_table = PostgresOperator(
        ...
        (
            ...
            lead_owner_id varchar
               (
                   512
               ),
            lead_sub_status varchar
               (
                   512
               ),
            lead_last_call_status varchar
               (
                   512
               ),
            lead_last_call_sub_status varchar
               (
                   512
               ),
            lead_last_call_connection_status varchar
               (
                   512
               ),
            reactivation_bucket varchar
               (
                   512
               ),
            reactivation_date TIMESTAMP,
            source_intended_course varchar
               (
                   512
               ),
            intended_course varchar
               (
                   512
               ),
            ...
            notable_event_description varchar
               (
                   10000
               ),
            ...
            mx_custom_10 varchar
               (
                   10000
               ),
            mx_custom_11 varchar
               (
                   512
               ),
            mx_custom_12 varchar
               (
                   512
               ),
            mx_custom_13 varchar
               (
                   512
               ),
            mx_custom_14 varchar
               (
                   512
               ),
            mx_custom_15 varchar
               (
                   512
               ),
            mx_custom_16 varchar
               (
                   512
               ),
            mx_custom_17 varchar
               (
                   512
               ),
            mx_priority_status varchar
               (
                   512
               ),
            mx_rfd_date varchar
            ...
            mx_identifer varchar
               (
                   512
               ),
            mx_organic_inbound varchar
               (
                   512
               ),
            mx_entrance_exam_marks varchar
               (
                   512
               ),
            mx_lead_quality_grade varchar
               (
                   512
               ),
            mx_lead_inherent_intent varchar
               (
                   512
               ),
            mx_test_date_n_time TIMESTAMP,
            mx_lead_type varchar
               (
                   512
               ),
            mx_utm_source varchar
               (
                   512
               ),
            mx_utm_medium varchar
               (
                   512
               ),
            score varchar
            ...
            mx_phoenix_identifer varchar
               (
                   512
               ),
            mx_phoenix_lead_assigned_date TIMESTAMP,
            mx_prospect_status varchar
               (
                   512
               ),
            mx_reactivation_source varchar
               (
                   512
               ),
            mx_reactivation_date TIMESTAMP,
            mx_lead_status varchar
               (
                   512
               ),
            mx_pmm_identifier varchar
               (
                   512
               ),
            mx_city varchar
...
transform_data = PostgresOperator(
        ...
        WHERE 
            TO_TIMESTAMP(sub.createdon_ist, 'YYYY-MM-DD HH24:MI:SS') >= current_date - interval '1440' Hour
        ),
            ...
            SELECT 
                ...
                (CASE when jsonb_typeof(activitydata) <> 'object' AND EXISTS ( SELECT 1 FROM jsonb_array_elements(activitydata) AS message
                    ...
                    THEN ( SELECT message->>'Value' FROM jsonb_array_elements(activitydata) AS message
                        WHERE (message->>'Key')::varchar = 'CurrentOwner' LIMIT 1 ) ELSE null end) AS assigned_current_owner
            FROM recent_activities
            WHERE eventname = 'LeadAssigned'
              AND (CASE when jsonb_typeof(activitydata) <> 'object' AND EXISTS ( SELECT 1 FROM jsonb_array_elements(activitydata) AS message
                        WHERE (message->>'Key')::varchar = 'CurrentOwner')
                    THEN ( SELECT message->>'Value' FROM jsonb_array_elements(activitydata) AS message
                        WHERE (message->>'Key')::varchar = 'CurrentOwner' LIMIT 1 ) ELSE null end) IS NOT NULL
        ),
        ...
        latest_lead_assigned AS (
            ...
            JOIN lead_assigned_owners la1 ON la1.relatedprospectid = ra.relatedprospectid 
        ...
        SELECT
            ...
            -- OPTIMIZED CURRENT_OWNER WITH FALLBACK
            COALESCE(
                (CASE when jsonb_typeof(activitydata) <> 'object' AND EXISTS ( SELECT 1 FROM jsonb_array_elements(activitydata) AS message
                        WHERE (message->>'Key')::varchar = 'CurrentOwner')
                    THEN ( SELECT message->>'Value' FROM jsonb_array_elements(activitydata) AS message
                        WHERE (message->>'Key')::varchar = 'CurrentOwner' LIMIT 1 ) ELSE null end),
        ...
        LEFT JOIN latest_lead_assigned lla ON lla.relatedprospectid = l.relatedprospectid 
