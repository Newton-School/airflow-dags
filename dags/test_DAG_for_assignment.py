...
def extract_data_to_nested(**kwargs):
    ...
    for transform_row in transform_data_output:
        pg_cursor.execute(
            ...
            'on conflict (table_unique_key) do update set question_completed_at = EXCLUDED.question_completed_at,'
...
dag = DAG(
    'test_assignment_question_user_mapping_DAG',
    default_args=default_args,
    description='A test DAG for assignment x users table transformation',
    catchup=False
)
    ...
    sql='''select
            ...
            from assignments_assignment
                left join courses_course 
                        on assignments_assignment.course_id = courses_course.id and (assignments_assignment.id between 1 and 2000)
