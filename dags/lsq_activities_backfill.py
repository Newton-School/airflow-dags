...
def extract_data_to_nested(**kwargs):
    ...
    for transform_row in transform_data_output:
        pg_cursor.execute(
            'INSERT INTO lsq_leads_x_activities_temp ('
              'table_unique_key,'
              'prospect_id,'
              'activity_id,'
              'email_address,'
...
create_table = PostgresOperator(
    ...
    sql='''CREATE TABLE IF NOT EXISTS lsq_leads_x_activities_temp (
            id serial,
...
transform_data = PostgresOperator(
    ...
    sql='''
        ...
        FROM (
            ...
            where 
                to_timestamp(l.createdon, 'YYYY-MM-DD hh24:mi:ss') > date_trunc('year', current_date) - interval '11' month
                and to_timestamp(l.createdon, 'YYYY-MM-DD hh24:mi:ss') <= date_trunc('year', current_date) - interval '5' month
        ) as l
        left join (
            select * from (
                select distinct
